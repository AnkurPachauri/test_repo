'''
 ...
 lclevy@free.fr 
 - 28 Aug 2013: initial version, 
 - Oct 2016: support for logins.json, 
 - Feb 2018: support for key4.db, 
 - Apr 2020: support for NSS 3.49 / Firefox 75.0 : https://hg.mozilla.org/projects/nss/rev/fc636973ad06392d11597620b602779b4af312f6
 - Oct 2025 : support for firefox 144 (by https://github.com/Banaanhangwagen, https://github.com/lclevy/firepwd/pull/29) for logins.json using AES256 instead of 3DES
...
# Use algo (not key length) to select cipher.
# Firefox 58 switched from key3.db to key4.db (SQLite/NSS 3.35) but still
# uses pbeWithSha1AndTripleDES-CBC, so decryptPBE returns a 24-byte key —
# the old len==32 check never matched and caused a premature sys.exit().
if algo == '1.2.840.113549.1.12.5.1.3':  # pbeWithSha1AndTripleDES-CBC (key3.db or key4.db pre-FF75)
    print('Using 3DES')
    final_key = key[:24]
    cipher_class = DES3
    block_size = 8
elif algo == '1.2.840.113549.1.5.13':  # pkcs5 pbes2 / AES-256-CBC (key4.db FF75+)
    print('Using AES-256')
    final_key = key[:32]
    cipher_class = AES
